/*
    ...
    https://learn.microsoft.com/pt-br/cpp/c-runtime-library/reference/mbstowcs-mbstowcs-l?view=msvc-170
   - conversão entre tipos char e wchar
    https://pubs.opengroup.org/onlinepubs/7908799/xsh/wchar.h.html - biblioteca
   wchar.h
    https://pubs.opengroup.org/onlinepubs/009696699/basedefs/wctype.h.html -
   biblioteca wctype.h
*/
...
char **ler_arquivo(const char *nome_arquivo, int *num_linhas) {
  ...
  *num_linhas = 0;
  ...
  while (fgets(buffer, sizeof(buffer), arq)) {
    ...
    if (temp == NULL) {
      printf("\n--ERRO AO REALOCAR PONTEIRO PARA A MUSICA--\n");
    ...
    if (texto[*num_linhas] == NULL) {
      printf("\n--ERRO AO ALOCAR PONTEIRO PARA A MUSICA--\n");
      for (int i = 0; i < *num_linhas; i++)
        free(texto[i]);
      free(texto);
      fclose(arq);
